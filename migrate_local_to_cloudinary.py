...
from app.models import AttendanceImage, FaceImage
...
def migrate_face_images(session, storage):
    ...
    stmt = select(FaceImage).where(
        (FaceImage.storage_provider != "CLOUDINARY")
        | (FaceImage.storage_provider.is_(None))
    )
    ...
    for rec in records:
        ...
        if not os.path.exists(local_path):
            print(
                f"Cảnh báo: File local không tồn tại ở đường dẫn '{local_path}'. Bỏ qua."
            )
            continue
        ...
        try:
            ...
            if metadata.get("storage_provider") == "CLOUDINARY":
                ...
                migrated_count += 1
                print(
                    f"Thành công: Đã chuyển đổi FaceImage ID {rec.image_id} -> {rec.secure_url}"
                )
            else:
                print(
                    f"Thất bại: Upload Cloudinary cho FaceImage ID {rec.image_id} không thành công."
                )
...
def migrate_attendance_images(session, storage):
    ...
    stmt = select(AttendanceImage).where(
        (AttendanceImage.storage_provider != "CLOUDINARY")
        | (AttendanceImage.storage_provider.is_(None))
    )
    records = session.exec(stmt).all()
    ...
    for rec in records:
        if not rec.image_path:
            print(
                f"Bỏ qua AttendanceImage ID {rec.image_id}: không có đường dẫn image_path."
            )
            continue
        ...
        try:
            ...
            if metadata.get("storage_provider") == "CLOUDINARY":
                ...
                migrated_count += 1
                print(
                    f"Thành công: Đã chuyển đổi AttendanceImage ID {rec.image_id} -> {rec.secure_url}"
                )
            else:
                print(
                    f"Thất bại: Upload Cloudinary cho AttendanceImage ID {rec.image_id} không thành công."
                )
        except Exception as e:
