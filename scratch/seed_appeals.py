...
from app.models import Appeal, Attendance, ClassSession, CourseRegistration
...
def main():
    with Session(engine) as session:
        ...
        session_item = session.exec(
            select(ClassSession)
            .where(ClassSession.class_section_id == 1)
            .order_by(ClassSession.class_session_id)
        ).first()
        ...
        for st_id in student_ids:
            ...
            if not att:
                ...
                session.exec(
                    text(
                        "SELECT setval('attendance_attendance_id_seq', (SELECT COALESCE(MAX(attendance_id), 0) + 1 FROM attendance), false);"
                    )
                )
                att = Attendance(
        ...
        existing_appeals = session.exec(
            select(Appeal).where(Appeal.status == "CHO_XU_LY")
        ).all()
        ...
        session.commit()
        print(
            f"Successfully created {created_count} demo appeals for TS. Nguyễn Văn An (gv2101)!"
        )
