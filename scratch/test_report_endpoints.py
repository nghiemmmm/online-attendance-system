...
def test_endpoints():
    with Session(engine) as session:
        ...
        my_classes_res = staff_service.read_my_class_sections(
            session=session, account_id=acc.account_id
        )
        print("\n=== 1. GET /staff/me/class-sections ===")
        print("Count:", my_classes_res.get("count", 0))
        for item in my_classes_res.get("data", []):
            print(
                f" -> ClassSection ID: {item.get('class_section_id')} | Students: {item.get('current_students')}"
            )

        # 2. Test read_my_reports
        reports_res = staff_service.read_my_reports(
            session=session, account_id=acc.account_id
        )
        ...
        for r in reports_res:
            print(
                f" -> Report ID: {r.get('id')} | Total Students: {r.get('totalStudents')} | Avg Rate: {r.get('averageAttendanceRate')}%"
            )
