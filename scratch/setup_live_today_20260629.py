...
from app.models import ClassSession, CourseRegistration, Attendance, ClassSection, AttendanceImage, Appeal
...
def main():
    with Session(engine) as session:
        ...
        for st_id in [21080001, 21080002]:
            ...
            if not reg:
                reg = CourseRegistration(class_section_id=1, student_id=st_id, status=True)
                session.add(reg)
            else:
                reg.status = True
                session.add(reg)
        ...
        print(f"ClassSession ID: {today_session.class_session_id} | Date: {today_session.class_date} | Status: {today_session.status}")
