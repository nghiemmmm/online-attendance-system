    ...
    with Session(engine) as session:
        ...
        today_session = session.exec(
            select(ClassSession).where(
                ClassSession.class_section_id == 1, ClassSession.class_date == today
            )
        ).first()
        ...
        print(
            f"Today ClassSession ID: {today_session.class_session_id} | Date: {today_session.class_date} | Status: {today_session.status}"
        )
