...
from app.models import ClassSection, CourseRegistration, Student, Staff, Course
...
def main():
    with Session(engine) as session:
        ...
        print(f"Student: {student.last_name} {student.first_name} (ID: {student.student_id})")
        ...
        for sec in sections:
            ...
            if not reg:
                ...
                session.add(reg)
                session.commit()
                print(f"-> Successfully registered for ClassSection #{sec.class_section_id} ({course_name})")
            else:
                print(f"-> Already registered for ClassSection #{sec.class_section_id} ({course_name})")
