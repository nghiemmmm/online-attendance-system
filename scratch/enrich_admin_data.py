...
def main():
    with Session(engine) as session:
        ...
        try:
            session.execute(
                text(
                    "SELECT setval('auditlog_ma_audit_log_seq', (SELECT COALESCE(MAX(ma_audit_log), 0) + 1 FROM auditlog), false);"
                )
            )
            session.commit()
        ...
        for mssv, name, email, phone in extra_students_data:
            ...
            st = session.exec(
                select(Student).where(Student.student_id == int(mssv))
            ).first()
            if not st:
                parts = name.split(" ")
                session.add(
        ...
        sample_logs = [
            (
                ...
                "Attendance",
                "36",
                {"student_name": "Nguyễn Đức Nghiêm", "status": "CO_MAT"},
            ...
            (
                ...
                "FaceImage",
                "102",
                {"student_name": "Trần Thị Mai", "action": "Duyệt ảnh mẫu AI"},
            ...
            (
                ...
                "3",
                {
                    "course": "Lập trình Di động Flutter",
            ...
            (
                ...
                "Appeal",
                "1",
                {"student": "Nguyễn Đức Nghiêm", "result": "CHAP_NHAN"},
