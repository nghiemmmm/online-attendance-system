...
def main():
    with Session(engine) as session:
        ...
        print(
            f"Setting up live active attendance session for Student {student_id} on {target_date}..."
        )
        ...
        if not reg:
            session.add(
                CourseRegistration(
                    class_section_id=1, student_id=student_id, status=True
                )
            )
            session.commit()
        ...
        print(
            f"Active Session ID: {today_session.class_session_id} | Date: {today_session.class_date} | Status: {today_session.status}"
        )
        ...
        print(
            "Database setup completed! The student dashboard now has the green active attendance button ready."
        )
