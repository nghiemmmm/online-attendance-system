...
from app.models import Student, CourseRegistration, ClassSection, ClassSession, Attendance, Appeal

def main():
    with Session(engine) as session:
        student_id = 21080001 # Nguyễn Đức Nghiêm
        print(f"Enriching database data for Student ID {student_id} (Nguyễn Đức Nghiêm)...")
        ...
        for cs_id in [1, 2, 3]:
            reg = session.exec(select(CourseRegistration).where(CourseRegistration.class_section_id == cs_id, CourseRegistration.student_id == student_id)).first()
            if not reg:
                session.add(CourseRegistration(class_section_id=cs_id, student_id=student_id, status=True))
        session.commit()
        
        # 3. Add rich past attendance records across sessions
        past_sessions = session.exec(select(ClassSession).where(ClassSession.class_date < date(2026, 6, 29))).all()
        for idx, sess in enumerate(past_sessions):
            att = session.exec(select(Attendance).where(Attendance.class_session_id == sess.class_session_id, Attendance.student_id == student_id)).first()
            ...
            if not att:
                session.add(Attendance(
                    ...
                    attendance_time=datetime.combine(sess.class_date, time(7, 32 if chosen_status=="CO_MAT" else 7, 48))
                ))
            else:
        ...
        att_absent = session.exec(select(Attendance).where(Attendance.student_id == student_id, Attendance.status == "VANG")).first()
        if att_absent:
            existing_appeal = session.exec(select(Appeal).where(Appeal.attendance_id == att_absent.attendance_id)).first()
