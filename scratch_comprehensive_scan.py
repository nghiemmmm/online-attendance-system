...
for filepath in py_files:
    ...
    for node in ast.walk(tree):
        ...
        if (
            ...
            issues_print.append(
                {"file": rel_path, "line": node.lineno, "code": ast.unparse(node)}
            )
        ...
        if isinstance(node, ast.AsyncFunctionDef):
            for subnode in ast.walk(node):
                if isinstance(subnode, ast.Call):
                    ...
                    if (
                        "time.sleep" in func_name
                        or "requests." in func_name
                        or func_name == "open"
                    ):
                ...
                for subnode in ast.walk(node):
                    ...
                    if isinstance(subnode, ast.Call):
                        ...
                        if any(
                            ...
                            "crud." in called
                            or "student_crud." in called
                            or "staff_crud." in called
                            or "class_session_crud." in called
                        ):
        ...
        if isinstance(node, ast.Call):
            ...
            if (
                called in ["async_sessionmaker", "SessionLocal", "SessionFactory"]
                and not rel_path.endswith("database.py")
...
with open(r"d:\TTCS\audit_scan_results.txt", "w", encoding="utf-8") as out:
    ...
    out.write(
        f"1. ROUTER LAYER VIOLATIONS (DB / CRUD DIRECT CALLS) ({len(issues_routers)})\n"
    )
    out.write(
        "------------------------------------------------------------------------\n"
    )
    ...
    for iss in issues_annotated:
        out.write(
            f"File: {iss['file']}:{iss['line']} in function {iss['func']}, arg '{iss['arg']}'\n"
        )
