...
def test_main_attendance_flow():
    ...
    try:
        with httpx.Client(base_url=BASE_URL) as client:
            ...
            login_data = {"username": "gv001", "password": "password"}
            resp = client.post("/auth/access-tokens", data=login_data)
    except httpx.ConnectError:
        pytest.skip(
            "Uvicorn server is not running, skipping E2E main attendance flow test."
        )
    ...
    with httpx.Client(base_url=BASE_URL) as client:
        ...
        if resp.status_code != 200:
            pytest.skip(
                "Không thể đăng nhập, hãy đảm bảo CSDL có tài khoản gv001 và mật khẩu 'password'"
            )
        ...
        resp_mo = client.patch(
            "/class-sessions/1", json={"status": "DANG_DIEN_RA"}, headers=headers
        )
        ...
        resp_dd = client.post(
            "/attendance-records/", json=attendance_data, headers=headers
        )
        assert resp_dd.status_code == 200, "Điểm danh tự động thất bại"
        assert resp_dd.json()["success"] is True

        # 4. Đóng điểm danh
        resp_dong = client.patch(
            "/class-sessions/1", json={"status": "DA_KET_THUC"}, headers=headers
        )
