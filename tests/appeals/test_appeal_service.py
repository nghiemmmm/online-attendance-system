...
def seed_test_data(session: Session):
    staff = Staff(staff_id=1, last_name="Nguyen", first_name="Giang", google_ten_dang_nhap="giang@example.edu", account_id=10)
    major = Major(major_id=1, major_name="Cong nghe thong tin")
    course = Course(course_id=701, course_name="Nhap mon AI", credit_count=3, status=True)
    session.add_all([staff, major, course])
    session.commit()

    student_account = Account(account_id=2, username="sv001", role="STUDENT", password_hash="hashed-password")
    student = Student(student_id=1, last_name="Le", first_name="An", major_id=1, account_id=2)
    session.add_all([student_account, student])
    session.commit()

    lop = ClassSection(class_section_id=1, course_id=701, staff_id=1, semester=1, academic_year="2025-2026")
    session.add(lop)
    session.commit()

    # 1. Buoi hoc hop le (class_date hom nay, ket thuc 10:00)
    valid_class_session = ClassSession(class_session_id=1, class_section_id=1, class_date=date.today(), start_time=time(8, 0), end_time=time(10, 0))
    # 2. Buoi hoc qua han 48 gio
    expired_class_session = ClassSession(class_session_id=2, class_section_id=1, class_date=date.today() - timedelta(days=3), start_time=time(8, 0), end_time=time(10, 0))
    session.add_all([valid_class_session, expired_class_session])
    session.commit()

    valid_attendance = Attendance(attendance_id=1, student_id=1, class_session_id=1, status="VANG")
    expired_attendance = Attendance(attendance_id=2, student_id=1, class_session_id=2, status="VANG")
