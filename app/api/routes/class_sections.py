...
from app.models import ClassSection, ClassSectionCreate, ClassSectionUpdate, ClassSectionPublic, ClassSectionsPublic
from app.crud import class_section_crud
from app.services.class_section_service import ClassSectionService, get_class_section_service
...
def read_class_sections(
    ...
    items, count = class_section_crud.get_class_sections(session=session, skip=skip, limit=limit)
...
def read_class_section(
    ...
    item = class_section_crud.get_class_section(session=session, class_section_id=class_section_id)
    if not item:
        raise HTTPException(status_code=404, detail="Lớp học phần không tồn tại")
    return item


@router.get("/{class_section_id}/students", dependencies=[Depends(get_current_active_lecturer)])
...
@router.get("/{class_section_id}/statistics", dependencies=[Depends(get_current_active_lecturer)])
...
@router.get("/{class_section_id}/warnings", dependencies=[Depends(get_current_active_lecturer)])
...
def create_class_section(
    ...
    new_section = class_section_crud.create_class_section(session=session, item_create=item_in)
    generate_sessions_for_class_section(session=session, class_section_id=new_section.class_section_id)
    return new_section


@router.post("/{class_section_id}/generate-sessions", dependencies=[Depends(get_current_active_superuser)])
def generate_class_section_sessions(
    ...
    if not item:
        raise HTTPException(status_code=404, detail="Lớp học phần không tồn tại")
    count = generate_sessions_for_class_section(session=session, class_section_id=class_section_id)
    return {"message": f"Đã tự động sinh {count} buổi học mới thành công", "count": count}


@router.patch("/{class_section_id}", response_model=ClassSectionPublic, dependencies=[Depends(get_current_active_superuser)])
def update_class_section(
    ...
    if not item:
        raise HTTPException(status_code=404, detail="Lớp học phần không tồn tại")
    return class_section_crud.update_class_section(session=session, db_item=item, item_update=item_in)


@router.delete("/{class_section_id}", dependencies=[Depends(get_current_active_superuser)])
