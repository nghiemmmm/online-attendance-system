...
from app.utils.cloudinary import delete_from_cloudinary, upload_to_cloudinary
...
class CloudinaryStorageService(StorageService):
    ...
    def save_image(
        ...
        if upload_success and secure_url:
            ...
            logger.warning(
                "Cloudinary upload completely failed. Falling back to local storage."
            )
            return self.local_fallback.save_image(
                file_bytes, filename, folder, public_id
            )
    ...
    def delete_image(self, public_id: str, local_path: str | None = None) -> bool:
        ...
        if public_id and (
            public_id.startswith("uploads/") or public_id.startswith("dataset/")
        ):
            deleted_local = deleted_local or self.local_fallback.delete_image(
                public_id, None
            )
    ...
    def get_image_url(self, public_id: str, local_path: str | None = None) -> str:
        if local_path or (
            public_id
            and (public_id.startswith("uploads/") or public_id.startswith("dataset/"))
        ):
