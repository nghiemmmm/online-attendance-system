...
def cleanup_expired_attendance_evidence(days: int = 2) -> int:
            ...
            if img.image_path:
                ...
                if "cloudinary.com" in img.image_path:
                    try:
                        ...
                        logger.error(
                            f"Failed to extract/delete Cloudinary public_id for {img.image_path}: {e}"
                        )
                else:
                    ...
                    if os.path.exists(local_path):
                        try:
                            ...
                            logger.error(
                                f"Failed to remove local file {local_path}: {e}"
                            )
    ...
    if deleted_count > 0:
        logger.info(
            f"Auto-cleanup: Removed {deleted_count} attendance evidence images older than {days} days."
        )
